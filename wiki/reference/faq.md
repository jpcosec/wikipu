...
**A:** `wiki/` is current truth and durable documentation. `desk/` is the operational surface for active work such as boards, proposals, gates, and sessions; it is mutable and intentionally ephemeral. `see_also:` `wiki/standards/00_house_rules.md`
...
**A:** Human approval is required when the effects cross the topology boundary, such as touching external systems, shared infrastructure, or artifacts outside the repository. Changes that stay inside the repo and are reversible by git do not require a human gate. `see_also:` `wiki/standards/00_house_rules.md`

**Q:** What counts as inside the topology?
**A:** Inside the topology means the repository boundary itself: source, tests, wiki, plan docs, and other repo-local artifacts. If a change stays within that boundary and can be reverted through git, it remains inside the topology. `see_also:` `wiki/standards/00_house_rules.md`
