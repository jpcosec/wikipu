...
def test_detect_misplaced_folder(tmp_path: Path):
    ...
    assert any(
        p.operation == "relocate" and "wiki/concepts/" in p.rationale
...
def test_detect_stale_config(tmp_path: Path):
    ...
    report = detect_cleansing_candidates(graph_path)
    assert any(
        p.node_id == "file:config.json" and p.operation == "destroy"
        for p in report.proposals
    )
...
def test_detect_orphaned_test(tmp_path: Path):
    ...
    report = detect_cleansing_candidates(graph_path)
    assert any(
        p.node_id == "file:tests/test_foo.py" and p.operation == "destroy"
        for p in report.proposals
    )
