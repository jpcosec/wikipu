...
def test_untyped_io_detects_port_without_schema_ref() -> None:
    graph = make_graph(
        make_node(
            "file:src/writer.py",
            io_ports=[
                IOFacet(medium="disk", path_template="data/out.json", schema_ref=None)
            ],
        )
    )
...
def test_orphaned_plans_detects_drawers_with_no_code_edge() -> None:
    graph = make_graph(make_node("doc:drawers/feature_x.md", node_type="concept"))
    findings = OrphanedPlansCheck().run(graph)
    assert any(f.node_id == "doc:drawers/feature_x.md" for f in findings)
