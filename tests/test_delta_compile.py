...
from wiki_compiler.drafts import (
    detect_stale_nodes,
    write_stale_drafts,
    promote_draft_node,
)
...
def test_delta_compile_workflow(tmp_path: Path):
    ...
    written = ingest_raw_sources(
        raw_dir, drafts_dir, project_root=tmp_path, manifest_path=manifest_path
    )
    ...
    # 5. Write drafts
    written_v2 = write_stale_drafts(
        graph_path, manifest_path, drafts_dir, project_root=tmp_path
    )
