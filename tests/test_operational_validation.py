...
def test_validate_issue(tmp_path: Path):
    path = write(tmp_path / "plan_docs/issues/unimplemented/test.md", """
# Test Issue
**Explanation:** Why.
**Reference:** Nodes.
**What to fix:** Result.
...
""")
    ...
    bad_path = write(tmp_path / "plan_docs/issues/unimplemented/bad.md", "# Missing fields")
    report = validate_wiki_artifact(bad_path)
    assert report.is_valid is False
    assert any(f.rule_id == "issue/explanation" for f in report.findings)

def test_validate_gates(tmp_path: Path):
    path = write(tmp_path / "desk/Gates.md", "| gate_id | proposal | opened | description | status |")
    report = validate_wiki_artifact(path)
    assert report.is_valid is True
...
def test_validate_backlog(tmp_path: Path):
    path = write(tmp_path / "backlog/idea.md", """
# Idea
**Added:** 2026-04-10
**Description:** What.
**Why deferred:** Later.
