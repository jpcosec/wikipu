...
- resolved tasks `132-scaffold-sibling-kgdb-repo`, `133-externalize-kgdb-dependency`, `134-scaffold-sibling-ontology-repo`, and `135-externalize-ontology-dependency`: extracted `kgdb` and `ontology` into sibling repos, switched `wikipu` to local editable uv sources for `kgdb`, `ontology`, and `sldb`, removed the in-repo package trees, and tightened contract loading so split-package imports no longer eagerly pull unrelated surfaces during startup
- resolved tasks `101-scaffold-kgdb-package`, `102-move-kgdb-base-contracts`, `103-move-graph-utils-to-kgdb`, `104-move-query-modules-to-kgdb`, `105-add-kgdb-cli-entrypoint`, and `106-scaffold-ontology-package`: added in-repo `kgdb` and `ontology` package scaffolds, moved kgdb contracts/graph/query surfaces behind compatibility shims, exposed a working `kgdb` CLI, and updated packaging discovery so editable installs expose the new packages
- resolved tasks `107-move-ontology-domain-contracts` and `111-move-energy-audit-to-ontology`: moved domain contracts into `src/ontology/contracts/` with compatibility shims, updated `kgdb` node binding to use ontology facet contracts, moved energy audit logic into `src/ontology/energy/audit.py`, and kept the old `wiki_compiler` imports working through shims
- resolved tasks `108-move-owl-reasoner-to-ontology`, `109-move-owl-backend-to-ontology`, and `110-move-owl-auditor-to-ontology`: moved the OWL reasoning lane into `src/ontology/reasoning/`, including the reasoner, backend package, and OWL audit check, while preserving the old `wiki_compiler` import paths through shims
- resolved tasks `112-split-cleanser-detection-into-ontology`, `113-move-facet-registry-to-ontology`, `114-move-facet-injectors-to-ontology`, and `115-move-facet-validator-to-ontology`: moved cleansing detection into `src/ontology/cleansing/`, moved the facet registry/injectors/validator into `src/ontology/facets/`, and preserved the existing `wiki_compiler` APIs through shims while keeping filesystem application logic in `wiki_compiler.cleanser`
- resolved tasks `116-add-ontology-cli-entrypoint`, `117-split-context-between-kgdb-and-wiki-compiler`, `118-split-scanner-between-ontology-and-wiki-compiler`, and `119-split-builder-between-ontology-and-wiki-compiler`: added the package-owned `ontology` CLI, moved graph-neighborhood traversal into `kgdb.query`, moved Python entity extraction into `ontology.facets.scanner`, and moved builder-side compliance/document-edge enrichment into `ontology.facets.builder`
- resolved tasks `120-create-kgdb-store-adapter` through `131-remove-ontology-reasoning-and-energy-shims`: created the adapter layer for kgdb and ontology services, rerouted commands and `main.py` through adapters, removed the kgdb shim family, removed ontology contract/facet shims, removed ontology reasoning/energy shims, updated affected tests to the new public package surfaces, and fixed graph JSON compatibility plus topology/scanner regressions uncovered during cleanup
