...
#==========================================================
# LLM Prompt Tool - Linux/FreeBSD Adaptation
# 
...
declare -A TEMPLATE_PROPERTIES
declare -A OUTPUT_TEMPLATE_SECTIONS
...
function verify_dependencies() {
    ...
    if ! command -v curl &> /dev/null; then
        echo "ERROR: 'curl' is not installed or not in PATH."
        echo "curl is required for API communication."
        ((errors++))
    else
...
function show_template_info() {
    ...
    if [[ -z "${PROMPT_TEMPLATES[$quoted_template_name]}" ]]; then
        ...
        for name in "${!PROMPT_TEMPLATES[@]}"; do
            echo "  - $name"
        done
        exit 1
    fi
...
#==========================================================
# PLATFORM-SPECIFIC FUNCTIONS (Linux/FreeBSD)
#==========================================================

CLIPBOARD_CMD=""
NOTIFY_CMD=""

function detect_platform_commands() {
    if [[ -n "$CLIPBOARD_CMD" ]]; then return; fi
...
function linux_copy_to_clipboard() {
    local content="$1"

    detect_platform_commands

    if [[ -z "$CLIPBOARD_CMD" ]]; then
        log_verbose "No clipboard tool found. Skipping clipboard copy."
        return 1
    fi
    ...
    case "$CLIPBOARD_CMD" in
        ...
        xclip)
            ...
            ;;
        xsel)
            printf '%s' "$content" | xsel --clipboard
            ;;
        pbcopy)
...
function linux_show_notification() {
    local message="$1"
    ...
    case "$NOTIFY_CMD" in
        notify-send)
            notify-send "gems" "$message"
            ;;
    esac
}

function linux_select_template_gui() {
    local available_templates="$1"
    local default_template="$2"

    if command -v fzf &> /dev/null; then
        local selected=$(echo "$available_templates" | tr ',' '\n' | fzf --prompt "Select template: " --query "$default_template")
        echo "$selected"
...
function linux_launch_viewer_app() {
    local app="$1"
    local file_path="$2"
        ...
        case "$app" in
            Terminal|auto|"")
                if command -v gnome-terminal &> /dev/null; then
                    gnome-terminal -- bash -c "glow -p '$file_path'; read -p 'Press Enter to close...'"
                elif command -v konsole &> /dev/null; then
                    konsole -e bash -c "glow -p '$file_path'; read -p 'Press Enter to close...'"
                elif command -v xterm &> /dev/null; then
                    xterm -e bash -c "glow -p '$file_path'; read -p 'Press Enter to close...'"
                else
                    glow -p "$file_path"
                fi
                ;;
            xdg-open)
                xdg-open "$file_path" 2>/dev/null || glow -p "$file_path"
                ;;
        esac
    else
        cat "$file_path"
...
function linux_check_dependencies() {
    local errors=0

    detect_platform_commands

    if [[ -z "$CLIPBOARD_CMD" ]]; then
        log_verbose "WARNING: No clipboard tool found (wl-copy, xclip, xsel)."
        log_verbose "Install one of: wl-copy (Wayland), xclip (X11), or xsel"
    else
        log_verbose "✓ $CLIPBOARD_CMD found (clipboard support)"
    fi

    if [[ -z "$NOTIFY_CMD" ]]; then
        log_verbose "WARNING: No notification tool found (notify-send)."
        log_verbose "Install libnotify for desktop notifications."
    else
        log_verbose "✓ $NOTIFY_CMD found (notification support)"
    fi
...
function copy_to_clipboard() {
    local content="$1"
    linux_copy_to_clipboard "$content"
    linux_show_notification "LLM results copied to clipboard"
}
...
function macos_launch_viewer_app() {
    linux_launch_viewer_app "$1" "$2"
}

function macos_select_template_gui() {
    linux_select_template_gui "$1" "$2"
}
