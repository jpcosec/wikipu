...
def write_stale_drafts(
    ...
    for node_id in stale_ids:
        node = [n for n in iter_knowledge_nodes(graph) if n.identity.node_id == node_id][0]
    ...
    for source_path in sources_to_ingest:
        ...
        written.extend(ingest_raw_sources(
...
def promote_draft_node(
    ...
    for draft_file in drafts_dir.rglob("*.md"):
        ...
        if node and node.identity.node_id == node_id:
            # If promoting from wiki/drafts/, we want to update the node_id to be in wiki/
            ...
            if live_node_id != node_id:
                draft_content = draft_content.replace(f'node_id: "{node_id}"', f'node_id: "{live_node_id}"')

            # Update status to implemented if it was planned
            if node.compliance and node.compliance.status == "planned":
                draft_content = draft_content.replace('status: "planned"', 'status: "implemented"')
