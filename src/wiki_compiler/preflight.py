...
class PreflightFinding(BaseModel):
    ...
    rule_id: str = Field(
        description="The ID of the rule being evaluated (e.g. 'ID-4')."
    )
    ...
    action_override: str | None = Field(
        default=None,
        description="Recommended replacement action (e.g. 'gate', 'abort').",
    )
...
def evaluate_action_safety(
    ...
    if uncommitted and action_type in {"write", "edit", "update", "delete"}:
        return PreflightFinding(
            rule_id="OP-6",
            severity="error",
            message=f"Uncommitted changes exist. Commit before editing. Uncommitted: {uncommitted[:3]}...",
...
def select_minimal_energy_action(
    perturbation_type: str, target_id: str, candidates: list[str]
) -> str:
    """
    ID-2: Minimal Energy.
