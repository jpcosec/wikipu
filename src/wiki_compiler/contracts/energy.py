"""Energy and zone contracts owned by wikipu."""
...
class SystemicEnergy(BaseModel):
    ...
    perturbation_energy: float = Field(
        description="Energy contributed by systemic uncertainty and gate pressure."
    )
    ...
    fft_code_wiki_so: float = Field(
        default=0.0, description="Spectral overlap between code and wiki."
    )
    fft_code_wiki_corr: float = Field(
        default=0.0, description="Correlation between code and wiki spectra."
    )
    fft_code_within_so: float = Field(
        default=0.0, description="Average spectral overlap within code files."
    )
    fft_code_within_corr: float = Field(
        default=0.0, description="Average correlation within code files."
    )
    fft_complexity_score: float = Field(
        default=0.0, description="Complexity score from FFT variance."
    )
    fft_energy: float = Field(
        default=0.0, description="Energy contribution from FFT-based metrics."
    )
...
class EnergyReport(BaseModel):
    ...
    consistency_status: str | None = Field(
        default=None,
        description="OWL consistency check result (CONSISTENT or INCONSISTENT).",
    )
...
class ZoneContract(BaseModel):
    ...
    zone: str = Field(description="Zone name, for example raw, desk, or drawers.")
    ...
    response_action: str = Field(
        default="scan", description="Response action when perturbations are detected."
    )
