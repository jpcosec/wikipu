...
def calculate_systemic_energy(graph: nx.DiGraph, project_root: Path) -> SystemicEnergy:
    ...
    status = build_status_report(graph_path=None, project_root=project_root, graph=graph)
    perturbations = len(status.get("perturbations", []))
    ...
    perturbation_energy = perturbations * 5.0
    gate_energy = open_gates * 20.0
    
    total_score = node_energy + edge_energy + violation_energy + perturbation_energy + gate_energy
